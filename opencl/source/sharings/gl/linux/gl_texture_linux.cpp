...
//    auto clientContext = context->getDevice(0)->getRootDeviceEnvironment().getGmmClientContext();
    ...
    int image_width =0, image_height = 0, internal_format = 0;
    
    GLSharingFunctionsLinux *sharingFunctions = context->getSharing<GLSharingFunctionsLinux>();

    sharingFunctions->glGetTexLevelParameteriv(target, miplevel, GL_TEXTURE_WIDTH, &image_width);
    sharingFunctions->glGetTexLevelParameteriv(target, miplevel, GL_TEXTURE_HEIGHT, &image_height);
    sharingFunctions->glGetTexLevelParameteriv(target, miplevel, GL_TEXTURE_INTERNAL_FORMAT, &internal_format);

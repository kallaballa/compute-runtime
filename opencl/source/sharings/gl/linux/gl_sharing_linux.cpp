...
GLboolean GLSharingFunctionsLinux::initGLFunctions() {
    ...
    glLibrary.reset(OsLibrary::load(Os::glDllName));

    if (eglLibrary->isLoaded()) {
        glFunctionHelper eglGetProc(eglLibrary.get(), "eglGetProcAddress");
        GLGetCurrentContext = (*eglLibrary)["eglGetCurrentContext"];
        GLGetCurrentDisplay = (*eglLibrary)["eglGetCurrentDisplay"];
        pfnEglCreateContext = (*eglLibrary)["eglCreateContext"];
        pfnEglDeleteContext = (*eglLibrary)["eglDestroyContext"];
        eglMakeCurrent = (*eglLibrary)["eglMakeCurrent"];
        eglCreateImage = (*eglLibrary)["eglCreateImage"];
        eglDestroyImage = (*eglLibrary)["eglDestroyImage"];
